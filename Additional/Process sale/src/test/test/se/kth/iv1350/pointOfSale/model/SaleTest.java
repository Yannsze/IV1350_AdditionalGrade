...
public class SaleTest {
    ...
    @Test
    void testDiscountWithoutStrategyPrintsMessage() {
        Sale sale = new Sale();
        sale.addItem(new se.kth.iv1350.pointOfSale.dto.ItemDTO("Test", 1, "desc", 10.0, 0.06), 1);

        ByteArrayOutputStream outContent = new ByteArrayOutputStream();
        PrintStream originalOut = System.out;
        System.setOut(new PrintStream(outContent));

        sale.discount(0); // Ingen strategi satt

        System.setOut(originalOut);
        String output = outContent.toString();
        assertTrue(output.contains("No discount strategy set. No discount applied."), "Should print warning if no discount strategy is set.");
    }
}
